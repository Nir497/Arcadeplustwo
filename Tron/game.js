const CONFIG = {
  ...
  startPositions: {
    p1: { x: 15, y: 24, direction: "right" },
    p2: { x: 45, y: 24, direction: "left" },
  },
  colors: {
    default: {
      p1Head: "#00FFFF",
...
class TronGame {
  ...
  async startRound() {
    ...
    if (mode === "skip") {
      ...
      const controlsText = this.mode === "bot"
        ? `PLAYER 1 USES WASD OR ARROW KEYS. PLAYER 2 IS ${this.botDifficulty.toUpperCase()} BOT CONTROL. INPUTS LOCK ON KEYDOWN AND U-TURNS ARE REJECTED.`
        : "PLAYER 1 USES WASD. PLAYER 2 USES ARROW KEYS. INPUTS LOCK ON KEYDOWN AND U-TURNS ARE REJECTED.";
    ...
    const infoText = this.mode === "bot"
      ? "TRAILS BECOME PERMANENT WALLS THE MOMENT A CYCLE LEAVES A CELL. HARD BOT EVALUATES SPACE; EASY BOT CHOOSES RANDOM SAFE TURNS. PRESS NEXT TO ARM THE COUNTDOWN."
      : "TRAILS BECOME PERMANENT WALLS THE MOMENT A CYCLE LEAVES A CELL. BOTH RIDERS MOVE ON THE SAME TICK, SO CRASHES CAN END IN A DRAW. PRESS NEXT TO ARM THE COUNTDOWN.";
  ...
  render() {
    ...
    this.drawPlayer(this.players.p1, palette.p1Head, now);
    this.drawPlayer(this.players.p2, palette.p2Head, now);
  }

  drawPlayer(player, color, now) {
    const px = player.x * CONFIG.cellSize;
    const py = player.y * CONFIG.cellSize;
